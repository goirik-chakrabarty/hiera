...
# from timm.models.layers import Mlp as MLP
...
class MultiScaleAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        qkv = self.qkv(x).reshape(B, H * W, 3, self.num_heads, -1)#.type(torch.float16)
        ...
        # Transpose back
        x = x.transpose(1, 2)#.type(torch.float32)
        x = x.reshape(B, H, W, -1)
...
class Hiera(nn.Module):
    ...
    def __init__(
        self,
        ## The rest of the xx correspond to positional encoding which is handled in hieradet by absolute window encodeing
        #xx input_size: Tuple[int, ...] = (224, 224),
        # embed_dim: int = 96,  # initial embed dim
        # num_heads: int = 1,  # initial number of heads
        # stages: Tuple[int, ...] = (2, 3, 16, 3),
        # q_pool: int = 3,  # number of q_pool stages
        # q_stride: Tuple[int, ...] = (2, 2),
        #xx mask_unit_size: Tuple[int, ...] = (8, 8),  # must divide q_stride ** (#stages-1)
        ...
        sep_pos_embed: bool = False, # THis is probably handled by window_pos which is ON by default
    ):
        super().__init__()
